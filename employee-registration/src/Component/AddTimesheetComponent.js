...
import { TextField, Button, Box, Typography, Grid, FormControl, MenuItem, Select, InputLabel, Paper, IconButton } from '@mui/material';
...
function AddTimeSheetComponent() {
  ...
  const [weekStartDate, setWeekStartDate] = useState(format(startOfWeek(new Date()), 'yyyy-MM-dd'));
  const [weekEndDate, setWeekEndDate] = useState(format(endOfWeek(new Date()), 'yyyy-MM-dd'));
            ...
            <Grid item xs={12} sm={6} key={day}>
              <Box display="flex" alignItems="center" mb={2}>
                ...
                <Box ml={1}>
                  <IconButton 
                    color="secondary"
                    onClick={() => handleNotesChange(day)({ target: { value: '' } })}
                  >
                    <ClearIcon />
                  </IconButton>
                </Box>
